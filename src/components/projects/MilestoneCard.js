...
const MilestoneCard = ({ milestone, handleDeleteMilestone, handleEditMilestone }) => {
  ...
  const handleSave = () => {
    // Check if Complete Date is being set for the first time (P0 milestone completion)
    const isNewlyCompleted = !milestone["Complete Date"] && editedMilestone["Complete Date"];
    const isP0 = milestone["Milestone Priority"] === 0 || milestone["Milestone Priority"] === "0";
    
    // Trigger global animation BEFORE calling parent handler
    // (parent handler may cause re-render/unmount)
    if (isNewlyCompleted && isP0) {
      triggerSuccessAnimation('P0 Milestone Complete!');
    }
  ...
  return (
    <>
      ...
      <div className='w-[280px] mt-2'>
        <div className="prj-card p-1 bg-cyan-500 hover:bg-fuchsia-600 rounded-xl shadow-md mb-4 text-black cursor-pointer relative">
